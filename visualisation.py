...
def plot_visualisations():
    ...
    water_level_df = pd.read_csv(
        "" + os.getcwd() + os.sep + "data" + os.sep + "gr_p.txt"
    )

    total_production_df = pd.read_csv(
        "" + os.getcwd() + os.sep + "data" + os.sep + "gr_q1.txt"
    )

    rhyolite_production_df = pd.read_csv(
        "" + os.getcwd() + os.sep + "data" + os.sep + "gr_q2.txt"
    )

    temperature_df = pd.read_csv(
        "" + os.getcwd() + os.sep + "data" + os.sep + "gr_T.txt"
    )
    ...
    (p1,) = host.plot(
        total_production_df["year"],
        total_production_df.iloc[:, 1],
        color=color1,
        label="total production rate (tonnes/day)",
    )
    # p2 refers to the first y-axis (water level)
    (p2,) = par1.plot(
        water_level_df["year"],
        water_level_df.iloc[:, 1],
        color=color2,
        label="water level (m)",
    )
    ...
    (p3,) = par2.plot(
        temperature_df["year"],
        temperature_df.iloc[:, 1],
        color=color3,
        label="temperature (degC)",
    )
    # p4 refers to the third y-axis (production rate)
    (p4,) = host.plot(
        rhyolite_production_df["year"],
        rhyolite_production_df.iloc[:, 1],
        color=color4,
        label="rhyolite production rate (tonnes/day)",
    )
    ...
    fig.tight_layout()
    fig.suptitle(
        "Visualisation of the initially provided data\n", fontweight="bold", fontsize=16
    )
